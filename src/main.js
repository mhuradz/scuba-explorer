...
function updateHud() {
  if (!scene3d) return;
  let speedReadout = document.querySelector('.speed-readout');
  if (!speedReadout) {
    speedReadout = document.createElement('div');
    speedReadout.className = 'speed-readout';
    speedReadout.setAttribute('aria-label', 'Character speed');
    speedReadout.innerHTML = '<span>CHARACTER SPEED</span><strong id="speed-value">0.00 m/s</strong><div class="speed-track"><i id="speed-bar"></i></div>';
    document.querySelector('.minimal-dive')?.append(speedReadout);
  }
  ...
  const speed = document.querySelector('#speed-value'); if (speed) speed.textContent = `${(scene3d.speed || 0).toFixed(2)} m/s`;
  const speedBar = document.querySelector('#speed-bar'); if (speedBar) speedBar.style.width = `${clamp((scene3d.speed || 0) / 2.2 * 100, 0, 100)}%`;
