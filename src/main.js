...
const equipment = {
  ...
  dive_light: { slot: 'light', name: 'Wide-beam dive flashlight', price: 400, detail: 'Wide cone illuminates dark water', preview: 'https://w7.pngwing.com/pngs/177/1023/png-transparent-flashlight-beuchat-light-emitting-diode-underwater-diving-light-lantern-headlamp-lamp.png' },
...
function isUnlocked(map) {
  // Progression is based only on completed maps. Loadout and character
  // equipment may affect safety, but never block an already unlocked level.
  return map.id === 1 || save.completedMaps.includes(map.id - 1);
}
function equipped(slot) { return equipment[save.equipped?.[slot]] || null; }
function oxygenCapacity() { return equipped('tank')?.capacity || 100; }
...
function topbar(showBack = false) { return `<header class="topbar">${showBack ? '<button class="icon-btn" data-action="back" aria-label="Back to dock">←</button>' : ''}${brand()}<div class="top-actions">${showBack ? '<button class="ghost-btn" data-action="home">MAIN MENU</button>' : ''}</div></header>`; }

function menu() { return `<main class="game-front"><canvas id="scene" aria-label="Live underwater background"></canvas><div class="front-shade"></div><section class="front-content"><p>SCUBA EXPLORER</p><h1>Into the<br>blue.</h1><nav aria-label="Main menu"><button data-action="choose-map">PLAY <small>CHOOSE MAP</small></button><button data-action="settings">SETTINGS</button><button data-action="open-shop">MARKET</button><button data-action="character">CHARACTER <small>VIEW LOADOUT 360°</small></button></nav></section>${overlay()}</main>`; }
...
function shopItem([id, item]) { const preview = item.slot === 'scooter' ? '/assets/equipment/sea-scooter-preview.png' : item.slot === 'tank' ? '/assets/equipment/oxygen-tank-preview.png' : item.slot === 'fins' ? '/assets/equipment/scuba-flippers-preview.png' : '/assets/diver-renderings/diver-front.jpg'; const owned = save.ownedEquipment.includes(id); const isEquipped = save.equipped[item.slot] === id; const affordable = owned || item.price <= save.spendablePoints; const managing=state.shopTab==='loadout'||item.slot==='scooter'||item.slot==='tank'||item.slot==='fins'||item.slot==='light'; const action = !managing ? 'buy' : isEquipped ? 'unequip' : owned ? 'equip' : 'buy'; const label = !managing ? owned ? 'OWNED' : affordable ? 'BUY' : 'LOCKED' : isEquipped ? 'UNEQUIP' : owned ? 'EQUIP' : affordable ? 'BUY' : 'LOCKED'; const sale = item.originalPrice && item.originalPrice > item.price; return `<article class="shop-item ${owned ? 'owned' : ''} ${isEquipped ? 'equipped' : ''}">${sale ? '<span class="sale-badge">SALE</span>' : ''}<img class="shop-item-preview equipment-preview ${item.slot === 'tank' ? 'tank-preview' : ''}" src="${preview}" alt="${item.name} preview"><div class="shop-item-copy"><span class="item-slot">${item.slot.toUpperCase()}</span><h3>${item.name}</h3><p>${item.detail}</p></div><div class="shop-item-foot"><strong>${isEquipped ? 'EQUIPPED' : owned ? 'OWNED' : sale ? `<s>${item.originalPrice} pts</s> ${item.price} pts` : `${item.price} pts`}</strong><button class="small-btn" data-action="${action}" data-item="${id}" ${action === 'buy' && (!affordable || owned) ? 'disabled' : ''}>${label}</button></div></article>`; }
...
function setupScene() {
  ...
  const goal = makeFinishGoal(map); scene.add(goal); const surface = makeSurface(scene); const depthBands = makeDepthBands(scene, map);
  scene3d = { renderer, scene, camera, player, buoy: goal, goal, markers, fish, specialFish: [], whales: [], marineLife, sharks, bubbles, particles, rocks, surface, depthBands, freeCam: false, mapId: map.id, mapTheme: map.theme, mapBounds: { ...map.bounds } };
...
function makeDepthBands(scene, map) {
  const bands = [];
  const labelXs = [];
  for (let x = map.bounds.min + 4; x <= map.bounds.max - 4; x += 10) labelXs.push(x);
  const metersPerFoot = 0.3048;

  // Each guide represents a 10 ft depth interval. Repeat the label across
  // the guide so it remains readable as the diver moves through the scene.
  for (let feet = 10; feet <= 300; feet += 10) {
    const y = 5 - feet * metersPerFoot;
    const geometry = new THREE.BufferGeometry().setFromPoints([
      new THREE.Vector3(map.bounds.min - 40, y, 0),
      new THREE.Vector3(map.bounds.max + 40, y, 0)
    ]);
    const color = feet >= 100 ? 0xff5b5b : feet < 50 ? 0xffa64d : 0x63e38b;
    const line = new THREE.Line(geometry, new THREE.LineDashedMaterial({ color, dashSize: .7, gapSize: .42, transparent: true, opacity: .52 }));
    line.computeLineDistances();
    scene.add(line);

    const labels = labelXs.map(x => {
      const canvas = document.createElement('canvas');
      canvas.width = 180; canvas.height = 58;
      const ctx = canvas.getContext('2d');
      ctx.shadowColor = '#062733'; ctx.shadowBlur = 6;
      ctx.fillStyle = '#efffff'; ctx.font = 'bold 25px DM Mono'; ctx.fillText(`${feet} ft`, 10, 27);
      ctx.fillStyle = '#8fd8cf'; ctx.font = '16px DM Mono'; ctx.fillText(`${(1 + feet * metersPerFoot / 10).toFixed(2)} bar`, 88, 27);
      const label = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.CanvasTexture(canvas), transparent: true, depthWrite: false, depthTest: false }));
      label.scale.set(2.95, .95, 1);
      label.position.set(x, y + .72, .05);
      scene.add(label);
      return label;
    });
    bands.push({ feet, line, labels });
  }
  return bands;
}
function makeWorldSpeedMarker(scene, player) {
  const group = new THREE.Group();
  const canvas=document.createElement('canvas'); canvas.width=320; canvas.height=76; const ctx=canvas.getContext('2d');
  ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.shadowColor='#00141c'; ctx.shadowBlur=8; ctx.fillStyle='#ffffff'; ctx.font='bold 42px DM Mono'; ctx.fillText('0.00 m/s',160,38);
  ...
  sprite.scale.set(4.4,1.05,1); sprite.position.set(0,0,0); sprite.renderOrder=1000; group.add(sprite);
  group.userData={canvas,ctx,sprite}; group.position.copy(player.position); scene.add(group); return group;
}
...
function loadDiverModel(target) {
  new GLTFLoader().load(activeCharacter().url, gltf => {
    ...
    equipVisuals(target, { light: save.equipped.light });
...
function updateHud() {
  if (!scene3d) return;
  document.querySelectorAll('.dive-pause').forEach(button => button.remove());
  document.querySelector('.dive-goal-hud')?.remove();
  let gemHud = document.querySelector('.gem-hud');
  if (!gemHud) {
    gemHud = document.createElement('section');
    gemHud.className = 'gem-hud';
    gemHud.setAttribute('aria-label', 'Gems collected');
    gemHud.innerHTML = '<span class="gem-label">GEMS COLLECTED</span><div class="gem-list" id="gem-list" aria-hidden="true"><span class="gem-slot">◆</span><span class="gem-slot">◆</span><span class="gem-slot">◆</span></div>';
    document.querySelector('.minimal-dive')?.append(gemHud);
  }
  ...
  const gemSlots = document.querySelectorAll('.gem-slot'); gemSlots.forEach((slot, index) => slot.classList.toggle('collected', index < state.markers));
  const depth = document.querySelector('#depth-count'); if (depth) depth.textContent = `${Math.round(state.depth)}M`;
  const feet = Math.max(0, state.depth) * 3.28084;
  const depthMeters = Math.max(0, state.depth); const pressure = 1 + depthMeters / 10;
  const telemetrySpeed = document.querySelector('#telemetry-speed');
  const telemetryDepth = document.querySelector('#telemetry-depth');
  const telemetryPressure = document.querySelector('#telemetry-pressure');
  if (telemetrySpeed) telemetrySpeed.textContent = (scene3d.speed || 0).toFixed(2);
  if (telemetryDepth) telemetryDepth.textContent = Math.round(feet);
  if (telemetryPressure) telemetryPressure.textContent = pressure.toFixed(2);
...
function animateScene(now = performance.now()) {
  ...
  if(scene3d.preview){
    ...
    // Keep large wildlife out of the map selector preview; its gameplay depth
    // coordinates are established only after a dive starts.
...
function loop(now) {
  ...
  // Battery charge powers the scooter but does not act like stamina. Keep
  // powered movement at its rated speed; stamina conditioning applies only
  // when swimming without scooter power.
  const staminaCondition = scooterPowered ? 1 : energy < 20 ? .28 + energy / 20 * .35 : .63 + (energy - 20) / 80 * .37;
  ...
  const maxFeet = equipped('tank')?.maxDepthFeet || activeCharacter().maxDepthFeet;
  ...
  const telemetry=document.querySelector('.dive-telemetry');
  telemetry?.classList.toggle('warning', feet >= 50 && feet < 100);
  telemetry?.classList.toggle('critical', feet >= 100 || beyondEquipment);
...
const keyboardKeys = { KeyW: 'w', KeyA: 'a', KeyS: 's', KeyD: 'd', KeyQ: 'q', KeyE: 'e', ArrowUp: 'ArrowUp', ArrowDown: 'ArrowDown', ArrowLeft: 'ArrowLeft', ArrowRight: 'ArrowRight', ShiftLeft: 'Shift', ShiftRight: 'Shift' };
window.addEventListener('keydown', event => {
  const key = keyboardKeys[event.code] || (event.key.length === 1 ? event.key.toLowerCase() : event.key);
  if (keyboardKeys[event.code] || [' ', 'Escape'].includes(event.key)) event.preventDefault();
  if (key === 'Escape') { togglePause(); return; }
  if (key === 'e' && state.screen === 'dive' && !state.paused) { interact(); return; }
  state.keys[key] = true;
}, true);
