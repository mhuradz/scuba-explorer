...
export function createExploration(game, actions) {
  game.mapBounds = { min: -Infinity, max: Infinity };
  ...
  gradient.addColorStop(0,'#e5ffe8'); gradient.addColorStop(.2,'#64ff9c99'); gradient.addColorStop(1,'#35ff8800');
  ctx.fillStyle = gradient; ctx.fillRect(0,0,64,64);
  const glowMap = new THREE.CanvasTexture(canvas);
  function glow(mesh) {
    const halo = new THREE.Sprite(new THREE.SpriteMaterial({map:glowMap,transparent:true,depthWrite:false,blending:THREE.AdditiveBlending}));
    halo.scale.set(1.9,1.9,1); mesh.add(halo);
  }
  game.markers.forEach(m => glow(m.mesh));
  const rockGeo = new THREE.IcosahedronGeometry(1,2);
  const rockMat = new THREE.MeshStandardMaterial({color:0x465c58,roughness:1});
  const lootGeo = new THREE.IcosahedronGeometry(.24,1);
  ...
  function chunk(id) {
    ...
    const loot = new THREE.Mesh(lootGeo,lootMat); loot.position.set(id%2 ? 14 : -14,id%2 ? -6.5 : -3.2,1); glow(loot);
  ...
  return { update() {
    ...
    for(const [id,c] of chunks) {
      ...
      if(c.loot.visible && Math.hypot(p.x-(id*80+c.loot.position.x),p.y-c.loot.position.y)<1.1){claimed.add(id);c.loot.visible=false;actions.reward(75);}
    }
    for(const m of game.markers) if(!m.collected && Math.hypot(p.x-m.x,p.y-m.mesh.position.y)<1.2){m.collected=true;m.mesh.visible=false;actions.collect();break;}
  }};
}
